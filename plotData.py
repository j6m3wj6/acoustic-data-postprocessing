...
# Import data - data preprocessing
  ...
  def _get_bose(self):
    with open('bose.txt', 'r', encoding='UTF-8') as file:
      ...
      freq = data.iloc[:, 0]
      self.data['Frequency'] = [float(f.replace(',', '').strip()) for f in freq]
      data = data.iloc[:, [i%2==1 for i in range(len(data.columns))]]
      data.columns = curves
...
# Import data - plot initialize
  ...
  def plotKlippelData(self):
    ...
    for col in self.data.columns[1:]:
      line, = self.canvas.ax.plot(self.data['Frequency'], self.data[col], label=col) # marker='o'
  ...
  def plotLEAPData(self):
    ...
    for col in self.data.columns[1:]:
      self.canvas.ax.plot(self.data['Frequency'], self.data[col], label=col) # marker='o'
