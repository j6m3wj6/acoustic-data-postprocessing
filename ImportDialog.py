...
class ImportFile_Widget(QWidget):
	...
	def initUI(self):
		...
		self.tb_files.setHorizontalHeaderLabels(['Source', 'FileName', 'DateTime'])
...
class ImportDialog(QDialog):
	...
	def initUI(self):
		...
		dlg_btnBox.setOrientation(QtCore.Qt.Horizontal)
		dlg_btnBox.setStandardButtons(QDialogButtonBox.Cancel|QDialogButtonBox.Ok)
		...
		self.wg_importFile.btn_importAP.clicked.connect(lambda: self.importData("AP"))
		self.wg_importFile.btn_importLEAP.clicked.connect(lambda: self.importData("LEAP"))
		self.wg_importFile.btn_importKLIPPEL.clicked.connect(lambda: self.importData("KLIPPEL"))
	...
	def importData(self, source):
		...
		if (DATA and DATA.sequence): 
			...
			self.wg_importFile.tb_files.setItem(row, 0, QTableWidgetItem(source))
			self.wg_importFile.tb_files.setItem(row, 1, QTableWidgetItem(DATA.name))
			self.wg_importFile.tb_files.setItem(row, 2, QTableWidgetItem(DATA.get_import_time()))
