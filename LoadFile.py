...
class FileData():
    ...
    def print(self):
        print("\n\n-------------------------")
        print("\tName: %s \n\tSource: %s \n\tfile_path: %s" % (self.name, self.source, self.file_path))
...
def load_AP_fileData(path):
    ...
    if path.endswith('.xlsx'):
        ...
        DATA = FileData(filename, source="AP", file_path=path, import_time=dt.datetime.today())
        ...
        for key, value in data.items():
            ...
            for curveIndex in range(int(len(data[key].columns)/2)):
                ...
                curveData_new = CurveData(label=label, note=note, xdata=curve_x, ydata=curve_y, _type = _type)
                curveData_new.set_line(curve_x, curve_y, curveData_new.get_legend(), COLORS[(curveIndex+rdm)%8])
                curveDatas.append(curveData_new)
            
            if (not isline): continue
            if test_name in DATA.sequence: DATA.sequence[test_name].extend(curveDatas)
            else: DATA.sequence[test_name] = curveDatas
    else:
        pass
    return DATA
...
def load_LEAP_fileData(path):
    ...
    if path.endswith('.txt'):
        with open(path, 'r', encoding='UTF-8', errors='ignore') as file:
            ...
            filename = path[path.rfind('/')+1:path.rfind('.')].strip() # LEAP_Impedance
            test_name = filename
            DATA = FileData(filename, source="LEAP", file_path=path, import_time=dt.datetime.today())

            if test_name in DATA.sequence: pass
            else: DATA.sequence[test_name] = []
            ...
            rdm = random.randint(1, 100)
            curveData_val = CurveData(label=label, note=note, xdata=freq, ydata=val, _type = _type)
            curveData_val.set_line(freq, val, curveData_val.get_legend(), COLORS[(rdm)%8])
            curveDatas.append(curveData_val)

            curveData_phase = CurveData(label=label, note=note, xdata=freq, ydata=phase, _type = CurveType.Phase)
            curveData_phase.set_line(freq, phase, curveData_phase.get_legend(), COLORS[(1+rdm)%8])
...
def load_KLIPPEL_fileData(path):
    ...
    if path.endswith('.txt'):
        with open(path, 'r', encoding='UTF-8') as file:
            ...
            DATA = FileData(filename, source="KLIPPEL", file_path=path, import_time=dt.datetime.today())
            ...
            for i in range(int(len(data.columns)/2)):
                ...
                rdm = random.randint(1, 100)
                curveData_new = CurveData(label=labels[i], note=note, xdata=freq, ydata=val, _type = _type)
                curveData_new.set_line(freq, val, curveData_new.get_legend(), COLORS[(i+rdm)%8])
                curveDatas.append(curveData_new)
...
def load_Comsol_fileData(path):
    ...
    if path.endswith('.txt'):
        with open(path, 'r', encoding='UTF-8', errors='ignore') as file:
            ...
            filename = path[path.rfind('/')+1:path.rfind('.')] # LEAP_Impedance
            test_name = filename
            DATA = FileData(filename, source="COMSOL", file_path=path, import_time=dt.datetime.today())
    ...
    #         print(data.iloc[:,0])
            ...
            note = ""
            rdm = random.randint(1, 100)
            curveData_new = CurveData(label=test_name, note=note, xdata=freq, ydata=val, _type =  CurveType.FreqRes)
            curveData_new.set_line(freq, val, curveData_new.get_legend(), COLORS[(rdm)%8])
            curveDatas.append(curveData_new)
...
def determineTypeByTestName(test_name):
    if ("Phase" in test_name): return CurveType.Phase 
    elif ("Impedance" in test_name): return CurveType.IMP
    elif ("SPL" in test_name or "CEA" in test_name or 'RMS' in test_name): 
        return CurveType.FreqRes
    elif ("THD" in test_name): return CurveType.THD
