...
def load_file(_type):
    ...
    if dialog.exec_():
        ...
        path = file_name[0]
        if (_type == 'LEAP'):
            path, dataSequence = load_LEAP_file(path)
        elif (_type == 'AP'):
            path, dataSequence = load_AP_file(path)
        elif (_type == 'Klippel'):
            path, dataSequence = load_Klippel_file(path)
    else: pass
    return path, dataSequence
        

def load_LEAP_file(path):
    dataSequence = {}
...
AP_path, AP_DATA = load_AP_file("C:/Users/tong.wang/OneDrive - 國立台灣大學/桌面/Matlab tools/python/data/AP_Acoustic Response_all_xlsx.xlsx")
LEAP_path, LEAP_DATA = load_LEAP_file("C:/Users/tong.wang/OneDrive - 國立台灣大學/桌面/Matlab tools/python/data/LEAP_Impedance.txt")
KLIPPEL_path, KLIPPEL_DATA = load_Klippel_file("C:/Users/tong.wang/OneDrive - 國立台灣大學/桌面/Matlab tools/python/data/NFS_CEA2034.txt")
